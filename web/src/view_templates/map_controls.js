templates.map_controls = [
    '<div id="map_controls_inv_layer">',
    '   <div id="map_controls_compass">',
    '       <span><i class="fi-compass"></i></span>',
    '   </div>', // /#map_controls_compass
    '   <div id="map_controls_position_reset">',
    '       <span><i class="fi-arrows-in"></i></span>',
    '   </div>', // /#map_controlsposition_reset
    '   <div id="map_controls_zoom">',
    '       <div id="map_controls_zoom_in">',
    '           <span><i class="fi-plus"></i></span>',
    '       </div>',
    '       <div id="map_controls_zoom_out">',
    '           <span><i class="fi-minus"></i></span>',
    '       </div>',
    '   </div>', // /#map_controls_zoom
    '</div>'].join('\n');
